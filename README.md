...
├── match_predictor_model.keras         # Trained Keras model
...
- **ML Model:** TensorFlow/Keras Sequential Neural Network
...
| Issue | Solution |
|-------|----------|
| `Model file not found` | Ensure `match_predictor_model.keras` is in the root directory |
| Slow first load | TensorFlow initialization takes 10-15s on first run; model is cached after |
| `ModuleNotFoundError` | Run `pip install -r requirements.txt` |
| Streamlit Cloud crash | Check that `requirements.txt` has correct TensorFlow version constraints |
