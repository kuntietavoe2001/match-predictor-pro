...
@st.cache_resource
def load_model():
    """Load and cache the ONNX model — persists across reruns."""
...
def predict_with_model(session, input_data):
...
if st.button("PREDICT MATCH RESULT", type="primary", use_container_width=True):
    if model is None:
        st.error("Model file not found! Please ensure match_predictor_model.onnx is in the app directory.")
    else:
        ...
        base_prob = float(predict_with_model(model, padded).flatten()[0])
